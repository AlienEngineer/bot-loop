...
- **`bot-loop`** — a terminal UI to browse issues and start background bots.
...
3. Resolves merge conflicts on open PRs (hands the conflicts to the bot).
...
9. If the bot needs more info, posts the question, labels the issue `needs-info`,
...
| `--agents-model <model>` | `AGENTS_MODEL` | `claude-sonnet-4.5` | Model for the one-time AGENTS.md bootstrap. When the repo has no AGENTS.md / `copilot-instructions.md`, a read-only pass writes a short AGENTS.md and merges it onto the default branch so every future run gets it. Runs once per repo, so it defaults to a capable mid model. `off` disables the bootstrap. |
| `--issues-dir <dir>` | `ISSUES_DIR` | `<repo>/issues` | Folder scanned for issue markdown files. |
| `--quiet` | `QUIET` | off | Only write the bot's output to the per-run log files; do not stream it to stdout. |
...
| `--quality-assurance` / `--no-quality-assurance` | `QUALITY_ASSURANCE` | on | Ask the bot to add tests for each issue, written from the user's perspective. Aliases: `--qa` / `--no-qa`. Turn off to save cost. |
