...
impl LoopRunner {
    ...
    /// on `model` (`None` = auto). When `auto_merge` is set the loop is told to
    /// merge each PR automatically (`--auto-merge`, #135). Errors when a loop is
    /// already running or the process cannot be spawned. Returns the new process
    /// id.
    pub fn start(
        &mut self,
        script: &Path,
